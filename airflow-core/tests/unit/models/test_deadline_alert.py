...
DEADLINE_INTERVAL = 60.0
DEADLINE_CALLBACK_DEF = {"path": "test.callback"}
...
def deadline_alert_orm(dag_maker, session, deadline_reference):
    ...
    with time_machine.travel(DEFAULT_DATE, tick=False):
        alert = DeadlineAlert(
            ...
            callback_def=DEADLINE_CALLBACK_DEF,
        )
        session.add(alert)
        session.flush()
        return alert
...
class TestDeadlineAlert:
    ...
    def test_deadline_alert_equality(self, dag_maker, session, deadline_reference):
        with dag_maker(DAG_ID, session=session):
            pass
    ...
    def test_deadline_alert_hash(self, dag_maker, session, deadline_reference):
        with dag_maker(DAG_ID, session=session):
            pass
