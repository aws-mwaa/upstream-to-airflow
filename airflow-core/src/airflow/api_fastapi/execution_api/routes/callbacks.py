...
# The callback subprocess is minted a long-lived ``callback``-scoped token whose
# ``sub`` is the callback UUID. That token is accepted ONLY on this router, and
# ``callback:self`` pins it to its own callback id. It is not valid on any
# connections/variables/xcoms/dag-runs route — it can only reach the exchange
# endpoint below, which burns it exactly once for a short-lived ``execution``
# token used for the actual context read.
