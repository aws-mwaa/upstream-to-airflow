...
async def require_auth(
    ...
    # ``*:self`` scopes pin a token to a single resource: the JWT ``sub`` must equal the
    # resource id in the route's path. Each entry maps the Security scope to the path
    # parameter that carries that id; the first matching scope is enforced.
