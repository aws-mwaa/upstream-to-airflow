...
class TriggerRunnerSupervisor(WatchedSubprocess):
    ...
    def _create_workload(
        ...
        if trigger.task_instance is None:
            ...
            # ``callback`` is an attribute of the Trigger model, not the BaseEventTrigger
            # protocol — asset-only triggers (and spec'd test mocks) may not expose it.
            ...
            if callback and dag_run_data is None:
                ...
                if callback_data.get("dag_id") and callback_data.get("run_id"):
                    # Routing data present but the DagRun lookup failed (transient DB blip):
                    # skip for now, the trigger is retried on the next loop. Callbacks without
                    # dag_id/run_id (pre-3.3) fall back to the context stored in their kwargs.
                    log.warning(
    ...
    def _fetch_callback_dag_run_data(self, trigger: Trigger, *, session: Session) -> dict | None:
        ...
        a standard Context at runtime (same pattern as start_from_trigger tasks).
        """
        ...
        # Deadline metadata rides along so the runtime context can expose
        # context["deadline"] for templates like {{ deadline.deadline_time }}.
...
class TriggerRunner:
    ...
    def _build_context_from_dag_run_data(dag_run_data: dict) -> Context:
        ...
        Provides the same DagRun-level context fields (dag_run, run_id, logical_date, ds,
        ts, etc.) that task-bound triggers get via RuntimeTaskInstance.get_template_context(),
        ...
        dag_run_data = dict(dag_run_data)
        deadline = dag_run_data.pop("_deadline", None)
        dag_run = DRDataModel(**dag_run_data)
        return build_context_from_dag_run(dag_run, deadline=deadline)  # type: ignore[return-value]
            ...
            try:
                ...
                if ti := workload.ti:
                    ...
                    if workload.dag_run_data:
                        ...
                        # For callback triggers, build a standard Context and set it on the
                        # trigger instance — the same pattern as setting
                        # trigger_instance.task_instance = runtime_ti for task-bound triggers.
                        if isinstance(trigger_instance, CallbackTrigger):
                            trigger_instance._callback_context = self._build_context_from_dag_run_data(
                                workload.dag_run_data
                            )
            except (TypeError, ValueError) as err:
                # ValueError covers pydantic.ValidationError from a version-skewed or
                # malformed dag_run_data in _build_context_from_dag_run_data.
