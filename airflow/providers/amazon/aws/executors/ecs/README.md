...
There are a number of configuration options available, which can either be set directly in the airflow.config
...
3. Load any values explicitly provided through airflow.config or environment variables. These are checked with Airflow's config precedence.
