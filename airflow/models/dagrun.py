...
class DagRun(Base, LoggingMixin):
    ...
    def __init__(
        ...
        self.stats_tags = prune_dict({"dag_id": self.dag_id, "run_type": self.run_type})
        super().__init__()
    ...
    def _emit_duration_stats_for_finished_state(self):
        ...
        if self.state == State.SUCCESS:
            Stats.timing(f"dagrun.duration.success.{self.dag_id}", duration, tags=self.stats_tags)
            Stats.timing("dagrun.duration.success", duration, tags=self.stats_tags)
        elif self.state == State.FAILED:
            Stats.timing(f"dagrun.duration.failed.{self.dag_id}", duration, tags=self.stats_tags)
            Stats.timing("dagrun.duration.failed", duration, tags=self.stats_tags)
