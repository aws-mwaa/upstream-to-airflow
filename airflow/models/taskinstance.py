...
class TaskInstance(Base, LoggingMixin):
    ...
    def __init__(
        ...
        self.run_id = run_id
        self.stat_tags = prune_dict(
            {"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id}
        )
    ...
    def _run_raw_task(
        ...
        Stats.incr(
            f"ti.start.{self.task.dag_id}.{self.task.task_id}",
            # TODO: using tags=self.stat_tags here and below causes zombie tasks and I can't
            #  figure out why.  Either fix or remove the self.stat_tags definition in __init__
            tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id},
        )
    ...
    def handle_failure(
        ...
        Stats.incr(
            f"operator_failures_{self.operator}",
            tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id},
        )
        Stats.incr(
            "ti_failures",
