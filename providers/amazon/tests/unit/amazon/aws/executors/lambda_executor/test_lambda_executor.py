...
def set_env_vars():
    overrides: dict[tuple[str, str], str] = {
        ...
        (CONFIG_GROUP_NAME, AllLambdaConfigKeys.SQS_QUEUE_URL): DEFAULT_QUEUE_URL,
...
class TestAwsLambdaExecutor:
    ...
    def test_check_health_success(self, mock_executor):
        ...
        mock_executor.sqs_client.list_queues.return_value = {"QueueUrls": [DEFAULT_QUEUE_URL]}
        mock_executor.lambda_client.list_functions.return_value = {
            "Functions": [
                {
        ...
        assert mock_executor.sqs_client.list_queues.call_count == 1
        assert mock_executor.lambda_client.list_functions.call_count == 1
        assert mock_executor.IS_BOTO_CONNECTION_HEALTHY
    ...
    def test_check_health_lambda_fails(self, mock_executor):
        ...
        mock_executor.lambda_client.list_functions.return_value = {
            "Functions": [
                {
                    "FunctionName": "banana",  # Wrong name, will fail health check
                    "State": "Active",
                }
            ]
        }
        ...
        assert mock_executor.sqs_client.list_queues.call_count == 0
        assert not mock_executor.IS_BOTO_CONNECTION_HEALTHY

    def test_check_health_sqs_fails(self, mock_executor):
        mock_executor.IS_BOTO_CONNECTION_HEALTHY = False
        mock_executor.sqs_client.list_queues.return_value = {
            "QueueUrls": ["banana"]
        }  # Wrong name fails health check
        mock_executor.lambda_client.list_functions.return_value = {
            "Functions": [
                {
                    "FunctionName": DEFAULT_FUNCTION_NAME,
