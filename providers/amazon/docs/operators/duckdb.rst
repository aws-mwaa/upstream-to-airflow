...
auth: turning an Airflow AWS connection into the DuckDB secret that grants access to S3.
This means  there is no credential wiring in the Dag.
...
.. code-block:: python
    ...
    summarise = AwsDuckDBOperator(
        ...
        sql="""
            ...
            ) TO 's3://my-bucket/summary/revenue.parquet' (FORMAT PARQUET)
        """,
...
``credential_chain`` (default)
    ...
    This is also the strategy that keeps credential resolution inside DuckDB, which is what lets the
    SDK refresh them.
...
.. note::
    ...
    — and falls back to whatever DuckDB's own settings and environment provide.

    That fallback is narrower than it looks. DuckDB's ``httpfs`` extension reads credentials from the
    ...
    the ECS container credential provider, so a container-assigned task role — how a managed Airflow
    worker normally receives credentials — is invisible to it, and S3 access fails with an opaque
