...
class AwsLambdaExecutor(BaseExecutor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sqs_queue_url = conf.get(CONFIG_GROUP_NAME, AllLambdaConfigKeys.QUEUE_URL, fallback="")
    ...
    def sync_running_tasks(self):
        ...
        # TODO: PAGINATION!
        # there may be messages sent to the queue that cannot be removed (old tasks, garbage accidentally
        # sent to the queue), in those cases we may not receive legitimate messages from the queue so we
        # should read the full compliment of messages

        for message in messages:
            receipt_handle = message["ReceiptHandle"]
            body = json.loads(message["Body"])
            # TODO: wrap these in exception handling. As part of our API we expect at least these two fields
            # to be present.
            ...
            try:
                ...
                # REVIEWER: Is this a reasonable enough log? Should we raise it to error level? Do something else?
                self.log.warning(
                    "Received task %s from the queue which is not found in running tasks. Ignoring message.",
                    ser_task_key,
                )
                task_key = None
            ...
            if task_key:
                if return_code == 0:
                    ...
                    # TODO: In this case the Lambda started but failed at run time. We could consider
                    # retrying these tasks within the executor, because this _likely_ means the Airflow task
                    # did not run to completion, however we can't be sure so perhaps not retrying is the
                    # safest option.
                    self.fail(task_key)
                    self.log.info(
        ...
        with Stats.timer("lambda_executor.adopt_task_instances.duration"):
            ...
            if serialized_task_keys := [
                ...
                for ti, ser_task_key in serialized_task_keys:
                    # TODO: add exception handling here for if the task key is not valid
    ...
    def end(self, heartbeat_interval=10):
        ...
        while True:
            ...
            # REVIEW: should we add a timeout here? Or just spin indefinitely until the user sends a SIGTERM?
