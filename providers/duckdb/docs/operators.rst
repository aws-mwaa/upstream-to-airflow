...
input from object storage and writes its output back out:
...
.. code-block:: python
    ...
    aggregate = DuckDBExecuteQueryOperator(
        ...
        """,
        hook_params={"extensions": ["httpfs"], "memory_limit": "2GB", "threads": 4},
    )

Reading from object storage needs credentials. Rather than configuring those by hand, use the
provider for your storage backend — for example
:class:`~airflow.providers.amazon.aws.operators.duckdb.AwsDuckDBOperator`, which creates the DuckDB
S3 secret from an Airflow AWS connection.
...
.. code-block:: python
    ...
    @task
    def summarise():
        hook = DuckDBHook(extensions=["httpfs"])
        with hook.get_conn() as conn:
            conn.execute("CREATE TABLE sales AS SELECT * FROM read_parquet('s3://bucket/sales.parquet')")
