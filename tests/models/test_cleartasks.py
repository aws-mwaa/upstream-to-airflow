...
class TestClearTasks:
    ...
    def test_clear_task_instances_executor(self, dag_maker, state, new_executor):
        ...
        with create_session() as session:
            ...
            # Update the dag the dag after the task has been scheduled, to update the executor field.
            # After the clear task below, the TI should now contain the latest executor name
            with dag_maker(
                "test_clear_task_instances_executor",
                start_date=DEFAULT_DATE,
                end_date=DEFAULT_DATE + datetime.timedelta(days=10),
