...
class TestDeadlineTrigger:
    ...
    def test_evaluate_with(self, trigger_enum, trigger_kwargs):
        trigger = DeadlineTrigger(trigger_enum)

        with mock.patch.object(DeadlineTrigger, str(trigger_enum)) as mock_trigger:
            trigger.evaluate_with(**trigger_kwargs)

            mock_trigger.assert_called_once_with(**trigger_kwargs)

    @pytest.mark.db_test
    def test_dagrun_queued_at(self, dagrun):
        trigger = DeadlineTrigger(DeadlineTrigger.DAGRUN_QUEUED_AT).evaluate_with(dag_id=dagrun.dag_id)

        assert dagrun.queued_at == trigger

    @pytest.mark.db_test
    def test_dagrun_logical_date(self, dagrun):
        trigger = DeadlineTrigger(DeadlineTrigger.DAGRUN_LOGICAL_DATE).evaluate_with(dag_id=dagrun.dag_id)
