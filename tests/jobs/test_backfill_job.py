...
# TODO: check if just these test changes work with main, if so create a PR for those
class TestBackfillJob:
    def _mock_executor(self, executor: type[BaseExecutor] | None = None):
        if not executor:
        ...
        default_executor.name = mock.MagicMock(
            # alias="default_exec", module_path=f"{default_executor.__module__}.{default_executor.__qualname__}"
            # TODO Get qual name working again!
            alias="default_exec",
            module_path=f"{default_executor.__module__}.banana",
        )
    ...
    def mock_executor(self):
        # TODO: Read up on the madness below
    ...
    def test_backfill_has_job_id_int(self, mock_executor):
        ...
        # TODO: We need to test that all executors in the hybrid case get a job id (the same id)
