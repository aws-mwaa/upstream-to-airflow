...
class TestSuperviseCallbackFetchesContext:
    ...
    def test_fetched_context_reaches_the_callback(self, mock_start, _nondumpable):
        ...
        assert set(context) == {"dag_run"}
        dag_run_data = context["dag_run"]
        assert dag_run_data["dag_id"] == "test_dag"
        assert dag_run_data["dag_run_id"] == "test_run"
        assert "run_id" not in dag_run_data
        assert dag_run_data["state"] == "running"
        assert dag_run_data["logical_date"] == "2026-09-01T00:00:00Z"
