...
def _fetch_callback_context(
    ...
    dag_run_data = dag_run.model_dump(mode="json")
    # The stored snapshot serializes the Dag run as DAGRunResponse, which names the run id
    # "dag_run_id"; match it so a callback written against snapshots reads fresh context unchanged.
    dag_run_data["dag_run_id"] = dag_run_data.pop("run_id")
    context: dict = {"dag_run": dag_run_data}
